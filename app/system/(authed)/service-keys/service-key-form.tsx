...
import {
  type ServiceKeyActionState,
  createServiceKeyAction,
  updateServiceKeyAction,
} from "@/app/_actions/system-service-keys";
...
export function ServiceKeyForm({
  ...
  const action = isEdit
    ? updateServiceKeyAction.bind(null, initial!.id)
    : createServiceKeyAction;
    ...
    <form action={formAction} className="flex flex-col gap-4" noValidate>
      ...
      <Field label="Нэр" htmlFor="name" error={fe.name}>
        <input
          ...
          placeholder="Жишээ: Моторын тос солих"
