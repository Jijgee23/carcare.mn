    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-2">
        ...
        {hasAnswers ? (
          <button
            type="button"
            onClick={() => setAnswers({})}
            className="text-xs text-[var(--oc-muted3)] hover:text-[var(--oc-ink2)] shrink-0 px-2 py-1 rounded-md hover:bg-white/[0.05]"
          >
            Цэвэрлэх
          </button>
        ) : null}
      ...
      {schema.sections.map((section) => {
        ...
        return (
          ...
          >
            <h3 className="font-semibold text-sm text-[var(--oc-ink)]">{section.title}</h3>
      ...
      {positions ? (
        <div className="grid gap-3 sm:grid-cols-2">
          {positions.map((pos) => (
            <div
              key={pos.code}
              className="rounded-lg border border-[var(--oc-line)] bg-[var(--oc-panel)] p-3 flex flex-col gap-2"
            >
              <div className="text-xs font-medium text-[var(--oc-muted2)]">
                {pos.label}
              </div>
              <PreviewInputs
                item={item}
...
function PreviewInputs({
  ...
  return (
    <>
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm transition-colors ${
                  active
                    ...
                    : "bg-[var(--oc-panel2)] border-[var(--oc-line)] text-[var(--oc-muted2)] hover:bg-white/[0.08]"
                }`}
              >
                <span
                  className={`w-3 h-3 rounded-full border ${
                    active ? TONE_DOT[tone] : "border-[var(--oc-muted4)]"
                  }`}
                />
                {opt}
              </button>
      ...
      {item.type === "photo" ? (
        <div className="rounded-lg border border-dashed border-[var(--oc-line2)] bg-[var(--oc-panel2)] px-3 py-2.5 text-sm text-[var(--oc-muted3)]">
