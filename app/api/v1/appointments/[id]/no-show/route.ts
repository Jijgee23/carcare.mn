...
function commandErrorResponse(error: unknown) {
  ...
  console.error("[appointments/no-show]", error instanceof Error ? error.name : "UnknownError");
 ...
 * Thin adapter over the P2-B1 `markAppointmentNoShowCommand` — the same
 * command `app/_actions/appointments.ts`'s `markAppointmentNoShowAction`
 * calls.
 */
export async function POST(
  ...
  try {
    const result = await markAppointmentNoShowCommand({
      actor: { ...auth.user, workingBranchId: scopeResult.branchId ?? undefined },
      appointmentId: id,
    });
    return jsonOk({ ok: true, appointmentId: result.appointmentId, status: "NO_SHOW" });
  } catch (error) {
