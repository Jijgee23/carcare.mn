...
function commandErrorResponse(error: unknown) {
  ...
  console.error("[appointments/arrived]", error instanceof Error ? error.name : "UnknownError");
  return jsonError(500, "Серверийн алдаа гарлаа. Дахин оролдоно уу.");
}

/**
 * POST /api/v1/appointments/[id]/arrived
 * Permission: appointments.edit
 * Thin adapter over the P2-B1 `markAppointmentArrivedCommand` — the same
 * command `app/_actions/appointments.ts`'s `markAppointmentArrivedAction`
 * calls.
 */
export async function POST(
  ...
  try {
    const result = await markAppointmentArrivedCommand({
      actor: { ...auth.user, workingBranchId: scopeResult.branchId ?? undefined },
      appointmentId: id,
    });
    return jsonOk({ ok: true, appointmentId: result.appointmentId, arrived: true });
  } catch (error) {
